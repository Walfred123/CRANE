...
class crane(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.cross_modal_attention = CrossModalAttention(self.feat_embed_dim, self.embedding_dim)
    ...
    def forward(self, adj):
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            fused_feats = self.cross_modal_attention(image_feats, text_feats)
        else:
...
class CrossModalAttention(nn.Module):
    def __init__(self, feat_embed_dim, embedding_dim):
    ...
    def forward(self, visual_feats, textual_feats):
        ...
        # Concatenate features for cross-modal attention
        combined_feats = torch.cat([visual_proj.unsqueeze(1), textual_proj.unsqueeze(1)], dim=1)
